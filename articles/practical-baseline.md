...
そのため、この記事に書かれた基準で製品の動作を保証するものではありません。
...
その後、[IE の完全サポート終了](https://blogs.windows.com/windowsexperience/2021/05/19/the-future-of-internet-explorer-on-windows-10-is-in-microsoft-edge/) (2022年6月) の影響を受けて、IE でページを表示した際に警告バナーが表示されるようにしていました。

- [サイボウズ製品・サービスでの Internet Explorer 11 サポート終了について（2022/08/17 更新） | サイボウズからのお知らせ](https://cs.cybozu.co.jp/2022/007588.html)

## 利用可能機能基準が曖昧に

開発時に利用して良い Web 標準の機能基準については、2022年6月までは「IE11 = 最低保証ライン」という暗黙的な基準が存在していました。
しかし、IE をサポートしなくなった先で、Chrome / Edge / Safari / Firefox それぞれのサポート機能の差異に対して、「**どのブラウザのどこまでのバージョンを開発で利用可能とするか**」という疑問が上がりました。
...
開発時における利用可能機能基準ができて、それに則って警告バナーを出せるようになりましたが、最初はその管理がチームごとに行われている状態でした。
利用可能機能基準の更新を忘れると「ページによってサポートするブラウザのバージョンが違う」ことになってしまうので問題です。
そこで、「サポートするブラウザのバージョンを一元化するパッケージ」を作成して、持ち回りでバージョンを更新してメンテナンスしていました。

結果、**パッケージのオーナーシップが曖昧に**なってしまい、最終的に利用可能機能基準の更新が滞ってしまっていました。
...
Google Analytics Baseline Checker というものがあり、GA から取得した特定の形式のデータをこの Checker にかけることで、サイトのアクティブユーザデータに基づいた Baseline Target を確認することができます。
...
参考：[Baseline Checker で得られるデータの例](https://blog.sakupi01.com/dev/articles/thoughts-on-our-baseline#the-path-to-our-baseline-newly-available)

## 考えられる影響

この結果は、「98% 以上のユーザはサポートできるようにしよう」というこれまでの内部基準に対して、Widely Available は従来よりもユーザーへのサポートが厳格な基準であることを示すものです。
...
特にアクセスログには、Bot・クローラー・攻撃者・etc の「非ユーザ」が大量に含まれており、古い `User-Agent` を意図的に送信してくるケースも考えられるため、**アクセスしてきたログが、私たちが「ユーザからのアクセス」と想定しているものとは限りませんでした**。
...
特定機能のサポート状況を正確に把握したければ、`User-Agent` からの推測ではなく、[Feature Detection](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Testing/Feature_detection) による実測が信頼性の高い手法になり得ます。
ユーザ環境での機能の利用可能状況を調べたいのであれば、Feature Detection 検証する以上に精度の高い方法はないでしょう。
...
古いバージョンをサポートすることは、単に「新しい機能を使うのを諦めるかどうか」という技術的な制約の問題として捉えられがちです。
しかし実際には、古いバージョンをサポートすることは **「ユーザの安全を担保できるか」という責任に直結する問題**でもあります。
つまり、これは**単なる「機能の有無」の話ではなく、セキュリティリスクを誰が負うのかという責任に直結する話**です。
...
開発時における利用可能機能の基準が Baseline であることにより、享受できるメリットも大きいです。
例えば、弊社の場合は以下のような恩恵を受けることができると考えられました。

- **開発時に利用可否を都度人力判断する手間がなくなる**
...
それらに責任を持つ上で、独自基準では見落としがちなリスクに対しても、Web プラットフォーム全体で十分に信頼可能な基準として認められた「Baseline」は非常に有効な選択肢となり得ます。
