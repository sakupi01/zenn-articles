...
しかし、そうした抽象化は「デザインパターン」と深く関係しており、Reactoneのコードベースではデザインパターンが適切に組み込まれたり改良されたりしていることがわかってきました。デザインパターンは、大規模プロダクトのフロントエンド刷新において保守・運用性や再利用性の向上に寄与している重要な要素の一つです。
