...
const GITHUB_RAW_BASE_URL =
  "https://raw.githubusercontent.com/sakupi01/zenn-articles/refs/heads/main/articles";
      ...
      frontendWeeklyArticles.map(async (article) => {
        ...
        try {
          ...
          if (!contentResponse.ok) {
            throw new Error(
              `Failed to fetch article content: ${contentResponse.statusText}`,
            );
          }
...
const fetchFromLocalFiles = async (fixtureDir: string): Promise<Blog[]> => {
  const articlesDir = path.join(process.cwd(), fixtureDir);
  ...
  try {
    ...
    console.error(
      `Error fetching frontend weekly articles from local files: ${error}`,
    );
    return [];
  }
};
